  ...
  test("that export of books works as intended") {
    ...
    Mockito.when(searchService.searchWithQuery(any[LanguageTag], any[Option[String]], any[Option[String]], any[Paging], any[Sort.Value]))
      .thenReturn(SearchResult(1L, 1, 1, Some(Language(TestData.LanguageCodeEnglish, "")),
        Seq(BookHit(1, "This is a title", "Short description", Language(TestData.LanguageCodeEnglish, ""), Seq(), None, Seq(), None, LocalDate.now(), "source", None, None, TestData.Api.DefaultLicense)))
      )
  ...
  test("that export of epubs creates a zip archive with all files") {
    ...
    Mockito.when(searchService.searchWithQuery(any[LanguageTag], any[Option[String]], any[Option[String]], any[Paging], any[Sort.Value]))
      .thenReturn(SearchResult(2L, 1, 2, Some(Language(TestData.LanguageCodeEnglish, "")),
        Seq(
          BookHit(1, "This is the first title", "Short description", Language(TestData.LanguageCodeEnglish, ""), Seq(), None, Seq(), None, LocalDate.now(), "source", None, None, TestData.Api.DefaultLicense),
          BookHit(2, "This is the second title", "Short description", Language(TestData.LanguageCodeEnglish, ""), Seq(), None, Seq(), None, LocalDate.now(), "source", None, None, TestData.Api.DefaultLicense)))
      )
