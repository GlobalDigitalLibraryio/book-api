...
class ChapterRepositoryTest extends IntegrationSuite with TestEnvironment with RepositoryTestHelpers {
  ...
  test("that Chapter.deleteChaptersExceptGivenSeqNumbers deletes chapters with seqNo not in the provided list, and no more") {
    withRollback { implicit session =>
      ...
      val chapter1 = chapterRepository.add(Chapter(None, None, translation.id.get, 1, Some("Chaptertitle1"), "Chaptercontent1", ChapterType.Content))
      val chapter2 = chapterRepository.add(Chapter(None, None, translation.id.get, 2, Some("Chaptertitle1"), "Chaptercontent1", ChapterType.Content))
      val chapter3 = chapterRepository.add(Chapter(None, None, translation.id.get, 3, Some("Chaptertitle1"), "Chaptercontent1", ChapterType.Content))
      val chapter4 = chapterRepository.add(Chapter(None, None, translation.id.get, 4, Some("Chaptertitle1"), "Chaptercontent1", ChapterType.Content))
      val chapter5 = chapterRepository.add(Chapter(None, None, translation.id.get, 5, Some("Chaptertitle1"), "Chaptercontent1", ChapterType.Content))

      chapterRepository.chaptersForBookIdAndLanguage(book.id.get, LanguageTag("eng")).size should equal(5)
      chapterRepository.deleteChaptersExceptGivenSeqNumbers(translation.id.get, Seq(1, 2, 3))
