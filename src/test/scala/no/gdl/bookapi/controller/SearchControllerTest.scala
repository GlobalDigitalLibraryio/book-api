...
class SearchControllerTest extends UnitSuite with TestEnvironment with ScalatraFunSuite{
  ...
  test("that search without lang searches for english books") {
    ...
    when(searchService.searchWithQuery(LanguageTag(BookApiProperties.DefaultLanguage), None, Paging(1,10), Sort.ByIdAsc)).thenReturn(result)
  ...
  test("that search with lang searches for books in correct lang") {
    ...
    when(searchService.searchWithQuery(LanguageTag("amh"), None, Paging(1,1), Sort.ByIdAsc)).thenReturn(result)
