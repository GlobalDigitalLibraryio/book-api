...
case class Pdf(language: LanguageTag, chapters: Seq[api.Chapter], translation: domain.Translation, source: Option[String] = None) {
  ...
  private def preprocessChapters(source: Option[String], chapters: Seq[api.Chapter]) = {
    def processStoryWeaverFirstPage(c: api.Chapter): api.Chapter = {
