...
import io.digitallibrary.bookapi.model.api.{BookForTranslation, CrowdinException, Error, Language, TranslateRequest, TranslateResponse}
import io.digitallibrary.bookapi.model.domain.{Paging, Sort, TranslationStatus}
import io.digitallibrary.bookapi.service.{ConverterService, ReadService, ReadServiceV2}
...
import org.scalatra.{InternalServerError, NoContent, NotFound}
import org.scalatra.swagger.{ResponseMessage, Swagger, SwaggerSupport}

import scala.util.{Failure, Success}

trait TranslationsControllerV2 {
  this: SupportedLanguageService with TranslationService with SynchronizeService with ReadServiceV2 with ConverterService =>
  ...
  class TranslationsControllerV2 (implicit val swagger: Swagger) extends GdlController with SwaggerSupport {
    ...
    private val getChapterForTranslation = (apiOperation[api.ChapterV2]("List supported languages")
    ...
    private val listAllTranslatedBooks = (apiOperation[api.SearchResultV2]("List all books that have been translated fully through translation system.")
    ...
    private val listAllProofreadBooks = (apiOperation[api.SearchResultV2]("List all books that have been proofread fully through translation system.")
    ...
    private def withTranslationStatus(status: TranslationStatus.Value)(implicit request: HttpServletRequest) = {
      ...
      readServiceV2.translationsWithTranslationStatus(
        status,
        Paging(
