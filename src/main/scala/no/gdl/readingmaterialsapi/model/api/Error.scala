...
object Error {
  ...
  val GENERIC_DESCRIPTION = s"Ooops. Something we didn't anticipate occured. We have logged the error, and will look into it. But feel free to contact ${ReadingMaterialsApiProperties.ContactEmail} if the error persists."
  val VALIDATION_DESCRIPTION = "Validation Error"
  val INDEX_MISSING_DESCRIPTION = s"Ooops. Our search index is not available at the moment, but we are trying to recreate it. Please try again in a few minutes. Feel free to contact ${ReadingMaterialsApiProperties.ContactEmail} if the error persists."
...
class NotFoundException(message: String = "The reading material was not found") extends RuntimeException(message)
