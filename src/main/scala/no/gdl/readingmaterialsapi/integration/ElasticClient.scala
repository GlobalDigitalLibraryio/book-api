...
import no.gdl.readingmaterialsapi.model.api.GdlSearchException
...
object JestClientFactory {
  def getClient(searchServer: String = ReadingMaterialsApiProperties.SearchServer): NdlaJestClient = {
    ReadingMaterialsApiProperties.RunWithSignedSearchRequests match {
  ...
  private def getSigningClient(searchServer: String): NdlaJestClient = {
    ...
    val awsSigner = new AWSSigner(new DefaultAWSCredentialsProviderChain(), ReadingMaterialsApiProperties.SearchRegion, "es", clock)
