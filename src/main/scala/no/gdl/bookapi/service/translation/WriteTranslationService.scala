...
trait WriteTranslationService {
  this: ConverterService with InTranslationRepository with InTranslationFileRepository with TransactionHandler =>
  val writeTranslationService: WriteTranslationService

  class WriteTranslationService extends LazyLogging {
    def addTranslationWithFiles(inTranslation: InTranslation, files: Seq[InTranslationFile], translateRequest: TranslateRequest): Try[InTranslation] = {
      inTransaction { implicit session =>
        val persisted = for {
          ...
          _ <- Try(files.map(f => f.copy(id = None, revision = None, inTranslationId = persistedTranslation.id.get, translationStatus = "in_progress",  etag = None)).map(fh => inTranslationFileRepository.add(fh)))
    ...
    def newInTranslation(translateRequest: TranslateRequest, crowdinMeta: CrowdinFile, crowdinChapters: Seq[CrowdinFile], crowdinProject: String): Try[InTranslation] = {
      val persisted = for {
